...
class Command(BaseCommand):
    help = "Print current alignment state: rubric rules, correction counts, eval scores."
    ...
    def handle(self, *args, **options):
        ...
        for dim, data in dimensions.items():
            ...
            if rules:
                ...
                self.stdout.write("  (no rules yet — add corrections and run synthesize_rules)")

        # Correction counts
        total_corrections = SummaryCorrection.objects.count()
        pending_corrections = SummaryCorrection.objects.filter(synthesized=False).count()
        self.stdout.write(f"\n--- CORRECTIONS ---")
        self.stdout.write(f"Total: {total_corrections}  |  Pending synthesis: {pending_corrections}")
        if pending_corrections:
            self.stdout.write("  → Run: python manage.py synthesize_rules")

        # Recent evaluation scores
        evals = list(
            SummaryEvaluation.objects.order_by("-created_at")
            .values("overall_completeness", "overall_faithfulness")[:20]
        )
        self.stdout.write(f"\n--- EVALUATION SCORES (last {len(evals)}) ---")
        if evals:
            c_vals = [e["overall_completeness"] for e in evals if e["overall_completeness"] is not None]
            f_vals = [e["overall_faithfulness"] for e in evals if e["overall_faithfulness"] is not None]
            if c_vals:
                self.stdout.write(f"Avg completeness : {sum(c_vals)/len(c_vals):.2f}/5.0")
            if f_vals:
                self.stdout.write(f"Avg faithfulness : {sum(f_vals)/len(f_vals):.2f}/5.0")
        else:
            self.stdout.write("  No evaluations yet — run: python manage.py evaluate_summaries")
