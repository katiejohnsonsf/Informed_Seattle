...
@require_GET
def previous_legislation_by_group(request, style: str, group: str):
    """
    Render every previous Council Bill matching one "who's affected"
    stakeholder group — unpaginated, across the site's full history.
    ...
    # Same depth as previous-legislation/<style>/page/<n>/, so the same
    # relative-path math applies.
    root = "../../../../"
...
def distill_previous_legislation_by_group():
    """Yield one static page per style x stakeholder group (plus the
    "no data" bucket) so every group is a real, shareable, bookmarkable
    URL covering the bill's full history — not just whatever page it
    happened to land on."""
    from server.legistar.label_schema import ALL_DEFAULT_CONSTITUENCIES

    for style in SUMMARIZATION_STYLES:
        for group in ["__none__"] + ALL_DEFAULT_CONSTITUENCIES:
            yield {"style": style, "group": group}
