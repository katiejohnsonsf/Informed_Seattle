...
def _stakeholder_filter_label(group: str) -> str:
    """Display text for a stakeholder filter value — matches the wording
    used in the dropdown's "no data" option."""
    if group == "__none__":
        return "No “Who’s Affected” Data"
...
@require_GET
def previous_legislation_by_group(request, group: str):
    """
    Render every previous Council Bill matching one "who's affected"
    stakeholder group — unpaginated, across the site's full history, at a
    short top-level URL (e.g. /renters/) for easy sharing.

    This exists because the site is fully static (django-distill; there's
    no live server to filter a paginated list per-request on GitHub Pages).
    Client-side JS filtering can only ever see whichever page's bills are
    already in the DOM, so a filter that needs to reach across pages has to
    be its own pre-built static page instead — one per group, generated by
    distill_previous_legislation_by_group.

    Lives at the site root rather than nested under previous-legislation/
    because there's currently only one SummarizationStyle — style ambiguity
    isn't a real concern, and a bare group name is the shareable URL asked
    for. If a second style is ever added, this will need to either pick a
    default explicitly or move back under a style-scoped prefix.
    """
    style = SUMMARIZATION_STYLES[0]
    all_entries = _build_previous_bill_entries(style)
    matching = [e for e in all_entries if _stakeholder_filter_matches(e, group)]

    dates = [e["meeting_date"] for e in matching]
    # <group>/index.html is 1 level deep, same as previous-legislation/<style>/.
    root = "../"
...
def distill_previous_legislation_by_group():
    """Yield one static page per stakeholder group (plus the "no data"
    bucket) so every group is a real, shareable, bookmarkable URL covering
    the bill's full history — not just whatever page it happened to land
    on."""
    from server.legistar.label_schema import ALL_DEFAULT_CONSTITUENCIES

    for group in ["__none__"] + ALL_DEFAULT_CONSTITUENCIES:
        yield {"group": group}
